...
namespace Kipon.Excel.UnitTests.WriterImplementation.OpenXml.Types
{
    public class RowTest
    {
        ...
        {
            Assert.AreEqual(Row.getRow(5), 5);
        }

        [Test]
        public void RowExceptionTest()
        {
            Assert.Throws<ArgumentOutOfRangeException>(() => Row.getRow(-1));
            Assert.Throws<ArgumentOutOfRangeException>(() => Row.getRow(Row.EXCEL_TOTAL_MAXROWS + 1));
        }
